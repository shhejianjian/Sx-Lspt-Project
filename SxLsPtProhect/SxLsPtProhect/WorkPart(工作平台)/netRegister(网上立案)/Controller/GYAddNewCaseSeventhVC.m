...
-(void)longPressToDo:(UILongPressGestureRecognizer *)gesture
{
    if(gesture.state == UIGestureRecognizerStateBegan)
    {
        ...
        if (indexPath.row != self.imageDataArr.count) {
            ...
            [HJHttpTool post:wsla_deletePhotoUrl ticket:ticket params:params success:^(id json) {
                NSLog(@"json:%@",json);
...
- (void)imagePicker:(LDImagePicker *)imagePicker didFinished:(UIImage *)editedImage{
    ...
    NSString *ticket = [[NSUserDefaults standardUserDefaults]objectForKey:@"login_ticket"];
    [HJHttpTool uploadImage:wsla_savePhotoUrl andImageData:imageData ticket:ticket params:params success:^(id json) {
...
- (void)loadWslaAjxxDetailInfoWithMlid{
    ...
    params[@"mlid"] = @"14";
    NSString *ticket = [[NSUserDefaults standardUserDefaults]objectForKey:@"login_ticket"];
    [HJHttpTool post:wsla_ajxx_detailClxxInfoUrl ticket:ticket params:params success:^(id json) {
        NSLog(@"%@===+++===%@",json,params);
        GYLoginModel *loginModel = [GYLoginModel mj_objectWithKeyValues:json[@"parameters"]];
        if ([loginModel.success isEqualToString:@"true"]) {
...
//                [MBProgressHUD showSuccess:loginModel.msg];
            ...
            for (GYClxxModel *clxxModel in arr) {
                ...
                [HJHttpTool postImage:wsla_ajxx_detailClxxImageInfoUrl ticket:ticket params:imageParams success:^(id json) {
...
- (IBAction)nexBtnClick:(id)sender {
    ...
    if (self.imageDataArr.count == 0) {
        ...
        params[@"ajbs"] = self.ajbsStr;
        NSString *ticket = [[NSUserDefaults standardUserDefaults]objectForKey:@"login_ticket"];
        [HJHttpTool post:wsla_ajxxtjUrl ticket:ticket params:params success:^(id json) {
            NSLog(@"json:::%@",json);
            GYLoginModel *loginModel = [GYLoginModel mj_objectWithKeyValues:json[@"parameters"]];
            if ([loginModel.success isEqualToString:@"true"]) {
                [MBProgressHUD hideHUDForView:self.view];
