...
- (IBAction)saveInfoBtnClick:(id)sender {
    ...
    if ([HJCheckInfoTool checkTelephone:self.firstViewLxdhTextField.text] == 0) {
...
- (IBAction)secondSaveInfoBtnClick:(id)sender {
    ...
    if ([HJCheckInfoTool checkTelephone:self.secondLxdhTextField.text] == 0) {
        [MBProgressHUD showError:@"手机号码填写不正确"];
        return;
    }
...
- (void)chooseZjlx {
    ...
    [HJHttpTool post:wsla_dwxzListInfoUrl ticket:ticket params:params success:^(id json) {
...
- (IBAction)thirdSaveInfoBtnClick:(id)sender {
    ...
    if ([HJCheckInfoTool checkTelephone:self.thirdViewLxdhTextField.text] == 0) {
        [MBProgressHUD showError:@"手机号码填写不正确"];
        return;
    }
