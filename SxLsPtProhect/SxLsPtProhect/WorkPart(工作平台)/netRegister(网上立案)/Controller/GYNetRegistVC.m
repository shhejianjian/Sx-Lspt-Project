...
@property (nonatomic, copy) NSString *clztType;
...
- (void)loadNewData
{
    ...
    self.currentPage = 1;
    [self loadWslaAjxxListInfoWithType:self.clztType];
}
- (void)loadMoreData
{
    self.currentPage ++;
...
- (void)loadWslaAjxxListInfoWithType:(NSString *)clztType{
    
    [MBProgressHUD showMessage:@"正在加载" toView:self.view];
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    params[@"pageSize"] = @"8";
    params[@"page"] = @(self.currentPage);
    params[@"clzt"] = clztType;
    NSString *ticket = [[NSUserDefaults standardUserDefaults]objectForKey:@"login_ticket"];
    [HJHttpTool post:wsla_ajxx_listInfoUrl ticket:ticket params:params success:^(id json) {
        NSLog(@"json:%@",json);
...
- (IBAction)daibanBtnClick:(id)sender {
    self.clztType = @"-1";
    [self loadNewData];
}

- (IBAction)yibanBtnClick:(id)sender {
    self.clztType = @"-2";
    [self loadNewData];
}
    ...
    if ([nrModel.clztmc isEqualToString:@"申请"] || [nrModel.clztmc isEqualToString:@"待审核"]) {
        ...
        [self showActionSheetWithTitle:@"温馨提示" message:@"请选择您的操作" appearanceProcess:^(SVAlertViewController * _Nonnull alertMaker) {
            ...
            if ([action.title isEqualToString:@"修改"]) {
                ...
                NSString *ticket = [[NSUserDefaults standardUserDefaults]objectForKey:@"login_ticket"];
                [HJHttpTool post:wsla_deleteUrl ticket:ticket params:params success:^(id json) {
                    [MBProgressHUD hideHUDForView:self.view];
                    NSLog(@"%@",json);
                    GYLoginModel *loginModel = [GYLoginModel mj_objectWithKeyValues:json[@"parameters"]];
