...
@interface HJHttpTool ()<MBProgressHUDDelegate,NSURLSessionDelegate>
...
+ (void)get:(NSString *)url ticket:(NSString *)ticket params:(NSDictionary *)params success:(void(^)(id json))success failure:(void(^)(NSError *error)) failure
{
    ...
    [manager GET:UrlString parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (success) {
            LSBaseModel *baseModel =  [LSBaseModel mj_objectWithKeyValues:responseObject];
            if(![baseModel.code isEqualToString:@"000000"]){
