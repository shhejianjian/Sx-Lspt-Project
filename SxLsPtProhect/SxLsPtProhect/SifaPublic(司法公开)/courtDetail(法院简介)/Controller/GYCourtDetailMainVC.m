...
- (void)viewDidLoad {
    ...
    if([self.pushType isEqualToString:@"1"]){
        [self firstBtnClick:self.firstBtn];
    } else if ([self.pushType isEqualToString:@"2"]){
        [self secondBtnClick:self.secondBtn];
    } else if ([self.pushType isEqualToString:@"3"]){
        [self thirdBtnClick:self.thirdBtn];
    } else if ([self.pushType isEqualToString:@"4"]){
        [self fourthBtnClick:self.fourthBtn];
    }
...
//    [SVProgressHUD showWithStatus:@"正在加载"];
    ...
    params[@"page"] = @"1";
    params[@"pageSize"] = @"100";
    [HJHttpTool post:xgmc_listInfoUrl ticket:@"" params:params success:^(id json) {
        NSLog(@"%@",json);
...
- (void)loadBmznListInfo {
    ...
    params[@"page"] = @"1";
    params[@"pageSize"] = @"100";
    [HJHttpTool post:bmzn_listInfoUrl ticket:@"" params:params success:^(id json) {
        NSLog(@"%@",json);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == self.secondTableView) {
        ...
        [HJHttpTool post:bmzn_detailInfoUrl ticket:@"" params:params success:^(id json) {
            NSLog(@"%@",json);
